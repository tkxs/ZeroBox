...
Android 版本会直接打开与浏览器一致的 ZeroAgent Gateway WebUI，包含聊天功能和账号会话。发布前，请在 GitHub Repository Variables 中设置 `ZEROAGENT_ANDROID_WEB_URL`，其值为已部署 Gateway 的 HTTPS 地址。未设置时，App 首次启动会要求填写 Gateway WebUI 地址，同样支持自建和本地部署。
